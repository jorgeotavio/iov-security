...
def start(train_dir, val_dir, result_dir, num_classes, num_epochs, use_gpu, use_data_percentage, batch_size):
    ...
    train_data = load_data(train_dir, train_transforms, use_data_percentage)
    val_data = load_data(val_dir, val_transforms, use_data_percentage)
    ...
    train_and_validate(model, num_epochs, train_loader, val_loader, criterion, optimizer, device)

    torch.save(model.state_dict(), result_dir+'train_result/result.pth')

if __name__ == '__main__':
    print('start train from src/main.py')
    # start()